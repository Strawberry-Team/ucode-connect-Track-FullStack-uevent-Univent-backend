...
async function createDatabase(connection: Connection, dbName: string): Promise<void> {
    await connection.query(`CREATE DATABASE IF NOT EXISTS \`${dbName}\``);
    console.log(`Database "${dbName}" created successfully.`);
}

async function createUser(connection: Connection, username: string, password: string): Promise<void> {
    await connection.query(`CREATE USER IF NOT EXISTS '${username}'@'%' IDENTIFIED BY '${password}'`);
    console.log(`User "${username}" created successfully.`);
}

async function grantPrivileges(connection: Connection, dbName: string, username: string): Promise<void> {
    await connection.query(`GRANT ALL PRIVILEGES ON \`${dbName}\`.* TO '${username}'@'%'`);
    console.log(`Privileges granted to user "${username}" for database "${dbName}"`);
}
...
async function configureMainDatabase(connection: Connection): Promise<string> {
    ...
    await createUser(connection, config.database.username, config.database.password);

    await grantPrivileges(connection, config.database.name, config.database.username);
...
async function configureTestDatabase(connection: Connection, appUsername: string): Promise<void> {
    // Import test config only when needed to avoid env conflicts
    const { default: testDatabaseConfig } = await import('./config/database.test.config');
    const testConfig = testDatabaseConfig();

    if (!testConfig.database.name) {
        console.log('Test database configuration not found or incomplete, skipping test database setup.');
        return;
    }
...
async function setupDatabase(): Promise<void> {
    ...
    try {
        ...
        // Step 1: Configure main database and get the app username
        const appUsername = await configureMainDatabase(connection);

        // Step 2: Configure test database with dynamic import to avoid env conflicts
