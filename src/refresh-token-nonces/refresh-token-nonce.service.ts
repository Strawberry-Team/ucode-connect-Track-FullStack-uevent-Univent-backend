...
export class RefreshTokenNonceService {
    ...
    async getRefreshTokenNonceByNonceAndUserId(
        userId: number,
        nonce: string,
    ): Promise<RefreshTokenNonce> {
        const NonceRes =
            await this.nonceRepository.findByRefreshTokenNonceAndUserId(
                userId,
                nonce,
            );
        if (!NonceRes) {
    ...
    async deleteRefreshTokenNoncesByUserId(userId: number): Promise<void> {
        return await this.nonceRepository.deleteRefreshTokenNoncesByUserId(
            userId,
        );
    }
