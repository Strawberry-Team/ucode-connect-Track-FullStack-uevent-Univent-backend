...
export class AuthService {
    ...
    async logout(userId: number, refreshNonceDto: string) {
        ...
        await this.refreshTokenNonceService.deleteRefreshTokenNonceByNonceId(
            nonceEntity.id,
        );
    ...
    async refreshToken(
        ...
        if (time - createdAt > convertToSeconds('1d')) {
            ...
            const nonceId: number = await this.refreshTokenNonceService
                .getRefreshTokenNonceByNonceAndUserId(userId, refreshNonce)
                .then((nonce) => nonce.id);
            await this.refreshTokenNonceService.deleteRefreshTokenNonceByNonceId(
                nonceId,
            );
    ...
    async resetPasswordWithConfirmToken(
        ...
        await this.refreshTokenNonceService.deleteRefreshTokenNoncesByUserId(
            userId,
        );
